...
def buildmodel():
    ...
    model = Sequential()
    model.add(Dense(30, input_shape=(45,), activation='relu'))  #
...
def trainNetwork(model,args):
    ...
    s_t = np.concatenate([x_t,[0,0],x_t,[0,0],x_t,[0,0],x_t,[0,0],x_t,[0,0]])[np.newaxis,:]
    ...
    while (True):
        ...
        if t % FRAME_PER_ACTION == 0:
            if random.random() <= epsilon:
                # print("----------Random Action----------")
                action_index = int(random.random()<0.2)
                # pdb.set_trace()
                a_t[action_index] = 1
            else:
                # pdb.set_trace()
                q = target_model.predict(s_t)       #input a stack of 4 images, get the prediction
                max_Q = np.argmax(q)
                action_index = max_Q
                a_t[max_Q] = 1
        ...
        if terminal:
            ...
            epoch += 1
            reward_epoch = 0
        s_t1 = np.concatenate([s_t[0,9:],x_t1,a_t])[np.newaxis,:]
        ...
        if t > OBSERVE:
            ...
            Q_sa_cur = model.predict(state_t1)
            
            # targets[range(BATCH), action_t] = reward_t + GAMMA*np.max(Q_sa, axis=1)*np.invert(terminal)
            # pdb.set_trace()
            targets[range(BATCH), action_t] = reward_t + GAMMA*Q_sa[range(BATCH),np.argmax(Q_sa_cur,axis=1)]*np.invert(terminal)
